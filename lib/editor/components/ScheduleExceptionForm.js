      ...
      <div>
        <Form>
          ...
          {activeEntity && activeEntity.exemplar === 'CUSTOM'
            ? <FormGroup
              controlId={`custom`}
              validationState={validationErrors.find(e => e.field === 'customSchedule') ? 'error' : undefined}
              className={`col-xs-12`}>
            ...
            {activeEntity && activeEntity.dates.length
              ? activeEntity.dates.map((date, index) => (
                <ExceptionDate
                  ...
                  key={index}
                  date={date}
                  validationState={validationErrors.find(e => e.field === `dates-${index}`) ? 'error' : undefined}
                  {...this.props} />
              ))

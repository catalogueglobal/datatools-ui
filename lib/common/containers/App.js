...
function wrapComponentInAuthStrategy (ComponentToWrap, requireAuth, requireAdmin) {
    ...
    render () {
      ...
      if (requireAuth && !user.token) {
        ...
        if (requireAdmin && true) {
          ...
          return (
            <ComponentToWrap
              {...pick(this.props, ['history', 'location', 'match'])}
              />
          )
        }
      }
...
export default class App extends Component {
  render () {
    ...
    const routes = [{
      ...
      path: '/public/feed/:feedSourceId/:feedVersionIndex?',
